...
app.post("/todo-backend/todos", (req, res) => {
  ...
  if (!todo || todo.trim() === "") {
    console.log(
      `[${new Date().toISOString()}] Invalid todo received: ${JSON.stringify(
        req.body
      )}`
    );
  ...
  // Check the length of the todo
  if (todo.length > 140) {
    console.log(
      `[${new Date().toISOString()}] Todo is too long. Maximum 140 characters.: ${JSON.stringify(
        req.body
      )}`
    );
    return res
      .status(400)
      .json({ error: "Todo is too long. Maximum 140 characters." });
  }
